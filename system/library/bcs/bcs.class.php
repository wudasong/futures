...
class BaiduBCS {
	...
	private function authenticate($opt) {
		...
		if (isset ( $opt ['fileUpload'] )) {
			...
			if (!isset($opt['extension']) && $headers ['Content-Type'] === 'application/x-www-form-urlencoded') {
		...
		if(isset($opt['extension'])){
			$mime_type = BCS_MimeTypes::get_mimetype ($opt['extension']);
			$headers ['Content-Type'] = $mime_type;
		}
	 ...
	 * @param string $bucket (Required) 
	 * @param string $object (Required) 
	 * @param string $content (Required); 需要上传的文件
	 * @param array $opt (Optional) 
	 * filename - Optional; 指定文件名
	 * acl - Optional ; 上传文件的acl，只能使用acl_type
	 * @return BCS_ResponseCore
